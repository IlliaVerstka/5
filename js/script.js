...
class CsSelect {
   init() {
      ...
      if (this.elements.length > 0) {
         ...
         for (let index = 0; index < this.objects.length; index++) {
            ...
            this.mediaHandler(mql, customSelect, customSelect.body)
            mql.addEventListener('change', e => this.mediaHandler(e, customSelect, customSelect.body))
   ...
   mediaHandler(e, customSelect, body) {
      if (e.matches) {
         slideDown(body, 0)
      } else {
         slideUp(body, 0)
         this.activeOption(customSelect.select.querySelector('.-select__item.-active'), customSelect.select)
      }
   }
   fillContent(obj) {
...
let sliderAds = null
disableSlider(sliderAds, '.slider-ads__body', {
   navigation: {
